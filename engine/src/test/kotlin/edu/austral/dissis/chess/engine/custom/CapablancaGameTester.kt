...
class CapablancaGameTester(private val runner: TestGameRunner) {
    ...
    fun debug(fileName: String): Stream<DynamicTest> {
        return gameTest("/custom_test_cases_capablanca/$fileName").let { Stream.of(it) }
    }
    ...
    private fun getTestPaths(): List<String> {
        val resourcePath = "custom_test_cases_capablanca/"
        ...
        uri?.let {
            ...
            Files.walk(path, 1).use { paths ->
                paths.filter { p -> Files.isRegularFile(p) }
                    .forEach { p: Path ->
                        testPaths.add("/custom_test_cases_capablanca/${p.fileName}")
                    }
            }
