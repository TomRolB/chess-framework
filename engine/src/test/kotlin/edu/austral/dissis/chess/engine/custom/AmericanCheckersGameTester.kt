...
class AmericanCheckersGameTester(private val runner: TestGameRunner) {
    ...
    fun debug(fileName: String): Stream<DynamicTest> {
        return gameTest("/custom_test_cases_american_checkers/$fileName").let { Stream.of(it) }
    }
    ...
    private fun getTestPaths(): List<String> {
        val resourcePath = "custom_test_cases_american_checkers/"
        ...
        uri?.let {
            ...
            Files.walk(path, 1).use { paths ->
                paths.filter { p -> Files.isRegularFile(p) }
                    .forEach { p: Path ->
                        testPaths.add("/custom_test_cases_american_checkers/${p.fileName}")
                    }
            }
